...
The v1.3 remediation and three-backend smoke are documented in [STAGED_V1_3_SMOKE_REPORT.md](docs/STAGED_V1_3_SMOKE_REPORT.md). The revised generator is SOL-approved, the safety regression suite passes, and all smoke records were independently reviewed by Luna. The follow-on namespaced pilot and preamble experiment are documented in [STAGED_NAMESPACED_PILOT_REPORT.md](docs/STAGED_NAMESPACED_PILOT_REPORT.md). The fictional Ted's flower-shop preamble was rejected after blind adjudication found merchant-role leakage; the production fallback is backend-namespaced seeds at temperature 0 with no preamble. The fallback produced 162 mechanically accepted records, 49 Luna-admitted records, and 45 globally deduplicated augmentation candidates. Synthetic output remains separate from the gold/certification corpus.
